...
**智能选项生成规则**：
1. 首先调用getGameStatus获取当前数值状态
2. 根据数值阈值动态调整选项：
   - 心理韧性≤20：移除"直接对抗"等高风险选项，增加"寻求帮助"选项
   - 心理韧性≥80：添加"巧妙反击"、"心理博弈"等强势选项
   - 学术进展≤20：导师威胁更有效，减少反抗选项
   - 学术进展≥80：可添加"我快毕业了"等底气十足的选项
   - 觉察水平≥80：解锁"识破套路"、"预判意图"等洞察选项
   - 金钱≥60（陆星河）：添加"经济解决"、"找关系"等选项
3. 不要在文本中描述"你当前的心理韧性是XX"，让玩家通过UI查看
4. 选项数量控制在3-5个，确保每个选项都有意义`,
      messages: messages,
      tools: {
        renderChoices: {
          description: "最重要的工具，每当用户需要做决策时必须调用。渲染选择按钮，玩家所有决策都必须通过此工具",
          parameters: z.object({
        ...
        timingChallenge: {
          description: "计时挑战工具，用于需要技巧和时机掌控的自主行动（如谨慎研究、社交技巧、计划执行等）",
          parameters: z.object({
        ...
        updateStats: {
          description: "【强制工具】更新学生的核心生存数值。任何数值变化都必须调用此工具，严禁在文本中描述数值变化",
          parameters: z.object({
            studentStats: z
              .object({
        ...
        setGameDay: {
          description: "【强制工具】设置当前游戏天数。当剧情需要推进时调用此工具",
          parameters: z.object({
